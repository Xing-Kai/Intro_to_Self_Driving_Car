...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
  ...
  // calcultate the average prob of every cell
  int height = grid.size();
  int width = grid[0].size();
  float prob_per_cell = 1.0 / ( (float) height * width);
  // initialize the grid with the average prob assigned to every cell
	...
	float p_miss) 
{
  ...
  // Initialize the new belief
  int height = beliefs.size();
  int width = beliefs[0].size();
  newGrid = zeros(height, width);

  // Updating the belief according to comparation of the sense result and the grid color
	...
	float blurring) 
{
  ...
  // Updating new belief after moving by dx/dy steps
  int i, j, new_i, new_j;
  for (i=0; i<height; i++) {
    for (j=0; j<width; j++) {
